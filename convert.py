...
with open("output.csv", "w") as f:
	...
	for poll in polls: 
		...
		date_format = "%m/%d/%Y"
		python_date = datetime.strptime(enddate, date_format)
		string_date = python_date.strftime("%d-%b-%y")
	    
    # 5. write a new row of data with the transformed date and value for "approve" 
		writer.writerow([string_date, poll["approve"]])